...
option_list = list(
  ...
  make_option(c("-A", "--altName"), action="store", default=NA,
...
#opt$tpmData = '/farm/scratch/rs-bio-lif/rabino01/evaRiising/CD9_Data/CD9_gene.tpm'
#opt$expData = '/farm/scratch/rs-bio-lif/rabino01/evaRiising/CD9_Data/CD9_gene.exp'
#opt$sampleData = '/farm/scratch/rs-bio-lif/rabino01/evaRiising/CD9_Data/CD9_sampleData.txt'
#opt$altName = '/farm/scratch/rs-bio-lif/rabino01/evaRiising/genomeData/mouse_ensembl_2_mouse_mgi.txt'
#opt$outPrefix = '/home/rabino01/test'
#opt$modelBatch = TRUE
...
if (ncol(expData) != nrow(sampleData)) {
  stop('Expected counts not present for all samples')
}
...
extractResults  <- function(DESeqObject, prefix = opt$outPrefix) {
  ...
  if (!is.na(opt$altName)) {
    ...
    resultData <- resultData[,c(7:8,1:6)]
  } else {
    resultData <- resultData[,c(7,1:6)]
  }
  # Extract and sort counts
  countData <- counts(DESeqObject, normalized=T)
  countData <- countData[row.names(resultData),]
  # Save results data
